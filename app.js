document.addEventListener('DOMContentLoaded', () => {
    ...
    const allElements = {
        ...
        promptArchitectDiv: document.getElementById('promptArchitect'),
        ...
        storyWeaverDiv: document.getElementById('storyWeaver'),
        storyTemplateDisplay: document.getElementById('story-template-display'),
        act1P: document.getElementById('act1'),
        act2P: document.getElementById('act2'),
        act3P: document.getElementById('act3'),
        elaborateBtn: document.getElementById('elaborateBtn'),
        thumbnailGeneratorDiv: document.getElementById('thumbnailGenerator'),
        thumbnailIdeasContainer: document.getElementById('thumbnailIdeasContainer'),
        pollGeneratorDiv: document.getElementById('pollGenerator'),
        pollQuestionContainer: document.getElementById('pollQuestionContainer'),
        socialAmplifierDiv: document.getElementById('socialAmplifier'),
        titleListUl: document.getElementById('titleList'),
    ...
    function populateSelect(selectElement, items, optional = false, isDefault = false) {
        let defaultOption = optional ? '<option value="none">-- None --</option>' : '<option value="random">-- Random --</option>';
        if (isDefault) defaultOption = `<option value="${items[0]}">${items[0]}</option>`;
        selectElement.innerHTML = defaultOption;
        items.sort().forEach(item => {
            if (isDefault && item === items[0]) return;
            const option = document.createElement('option');
            option.value = item;
    ...
    function generateStoryOutline() {
        ...
        let templateName = "Classic Showdown"; // Default
        ...
        allElements.storyTemplateDisplay.textContent = `~ ${templateName} Template Applied ~`;
        
        const template = data.scriptTemplates[templateName];
        allElements.act1P.textContent = template.act1.replace('{protagonist}', p.name).replace('{antagonist}', a.name).replace('{setting}', currentSelection.setting).replace('{motive}', currentSelection.motive).replace('{twist}', currentSelection.twist);
        allElements.act2P.textContent = template.act2.replace('{protagonist}', p.name).replace('{antagonist}', a.name).replace('{setting}', currentSelection.setting).replace('{motive}', currentSelection.motive).replace('{twist}', currentSelection.twist);
        allElements.act3P.textContent = template.act3.replace('{protagonist}', p.name).replace('{antagonist}', a.name).replace('{setting}', currentSelection.setting).replace('{motive}', currentSelection.motive).replace('{twist}', currentSelection.twist);
    }

    function elaborateStory() {
        generateStoryOutline();
        allElements.act1P.textContent += ` The air grows heavy, a palpable tension heralding the arrival of legends. The confrontation is not just a clash of power, but of will.`;
        allElements.act2P.textContent += ` The fight is a hurricane of motion and energy, a saga told in blows and blasts. But the true battle is one of endurance and strategy, as each combatant adapts to the other's impossible capabilities.`;
        allElements.act3P.textContent += ` The aftermath is silence. The victor stands not just over a fallen foe, but over a revealed truth that redefines their struggle, leaving a profound and startling question in its wake.`;
    }
    ...
    function generateSocialMediaContent() {
        ...
        allElements.titleListUl.innerHTML = `
            <li>${pName} vs. ${aName}: The Ultimate Showdown</li>
            <li>What If ${pName} Fought ${aName}? (An Eternal Myths AI Simulation)</li>
            <li>The EPIC Battle Between ${pName} and ${aName} Explained</li>
        `;
    ...
    function updateAllOutputs() {
        ...
        allElements.generationControls.style.display = 'block';
        Array.from(allElements.generatedContent.getElementsByClassName('module')).forEach(el => el.style.display = 'block');
    ...
    function generateLegend() {
        ...
        currentSelection.protagonist = getCharacterObject(pName, data.protagonists);
        currentSelection.antagonist = getCharacterObject(aName, data.antagonists);
        
        currentSelection.setting = allElements.settingSelect.value === 'random' ? getRandomItem(data.settings) : allElements.settingSelect.value;
        currentSelection.motive = allElements.motiveSelect.value === 'random' ? getRandomItem(data.motives) : allElements.motiveSelect.value;
        currentSelection.twist = allElements.twistSelect.value === 'random' ? getRandomItem(data.twists) : allElements.twistSelect.value;
        ...
        if (currentSelection.ceo !== 'none' && currentSelection.leader !== 'none') {
            story += ` The conflict, secretly funded by <strong>${currentSelection.ceo}</strong>, forces <strong>${currentSelection.leader}</strong> to intervene.`;
        } else if (currentSelection.ceo !== 'none') {
            story += ` Unbeknownst to them, the entire conflict is being orchestrated by <strong>${currentSelection.ceo}</strong>.`;
        } else if (currentSelection.leader !== 'none') {
            story += ` The outcome of their battle is being judged by <strong>${currentSelection.leader}</strong>.`;
        }
    ...
    function reroll(component) {
         switch(component) {
            ...
            case 'setting': currentSelection.setting = getRandomItem(data.settings); break;
            case 'motive': currentSelection.motive = getRandomItem(data.motives); break;
            case 'twist': currentSelection.twist = getRandomItem(data.twists); break;
        }
         while (currentSelection.protagonist.name === currentSelection.antagonist.name) {
            currentSelection.antagonist = getRandomItem(flatAntagonists);
        }
        let story = `Driven by <strong>${currentSelection.motive}</strong>, <strong>${currentSelection.protagonist.name}</strong> battles <strong>${currentSelection.antagonist.name}</strong> in <strong>${currentSelection.setting}</strong>. But the ultimate twist is: <strong>${currentSelection.twist}</strong>.`;
        allElements.resultP.innerHTML = story;
        updateAllOutputs();
    }
    ...
    function saveLegend() {
        ...
        const legendToSave = { 
            protagonist: currentSelection.protagonist.name,
            antagonist: currentSelection.antagonist.name,
    ...
    function renderLegends() {
        ...
        if (legends.length === 0) {
            allElements.hallOfLegendsContainer.innerHTML = '<p>Your saved legends will appear here.</p>';
            return;
        }
        legends.forEach(legend => {
            ...
            legendDiv.innerHTML = `
                ...
                <p><strong>${legend.protagonist}</strong> vs <strong>${legend.antagonist}</strong></p>
    ...
    function exportContent() {
        ...
        const content = `
MYTHOS ENGINE EXPORT
================================
LEGEND: ${pName} vs. ${aName}
--------------------------------
${allElements.resultP.innerText}

ART PROMPT
--------------------------------
${allElements.promptOutputTextarea.value}

SCRIPT OUTLINE (${allElements.storyTemplateDisplay.textContent.replace(/~/g, '').trim()})
--------------------------------
ACT I: ${allElements.act1P.textContent}
ACT II: ${allElements.act2P.textContent}
ACT III: ${allElements.act3P.textContent}

THUMBNAIL IDEAS
--------------------------------
${allElements.thumbnailIdeasContainer.innerText.replace(/(\r\n|\n|\r)/gm, "\n")}

COMMUNITY POLL
--------------------------------
${allElements.pollQuestionContainer.innerText}

YOUTUBE TITLES
--------------------------------
${allElements.titleListUl.innerText.replace(/(\r\n|\n|\r)/gm, "\n")}
...
${allElements.hashtagsTextarea.value}
    ...
    function initializeApp() {
        flatProtagonists = flattenCharacterList(data.protagonists);
        flatAntagonists = flattenCharacterList(data.antagonists);

        populateGroupedSelect(allElements.protagonistSelect, data.protagonists);
        populateGroupedSelect(allElements.antagonistSelect, data.antagonists);
        
        populateSelect(allElements.settingSelect, data.settings);
        populateSelect(allElements.motiveSelect, data.motives);
        populateSelect(allElements.twistSelect, data.twists);
        populateSelect(allElements.ceoSelect, data.ceos, true);
        populateSelect(allElements.leaderSelect, data.leaders, true);
        populateSelect(allElements.artStyleSelect, data.artStyles, false, true);
        populateSelect(allElements.moodSelect, data.moods, false, true);

        allElements.generateBtn.addEventListener('click', generateLegend);
        allElements.artStyleSelect.addEventListener('change', generateArtPrompt);
        allElements.moodSelect.addEventListener('change', generateArtPrompt);
        allElements.elaborateBtn.addEventListener('click', elaborateStory);

        allElements.copyPromptBtn.addEventListener('click', () => copyToClipboard(allElements.promptOutputTextarea, allElements.copyPromptBtn));
        allElements.copyDescBtn.addEventListener('click', () => copyToClipboard(allElements.youtubeDescTextarea, allElements.copyDescBtn));
        allElements.copyTagsBtn.addEventListener('click', () => copyToClipboard(allElements.hashtagsTextarea, allElements.copyTagsBtn));
